...
const CorrBadge = ({ c, testId }) => (
  <div data-testid={testId} className="rounded-md border border-[color:var(--line)] bg-[color:var(--void)]/40 px-3 py-2.5 min-h-[88px]">
    <div className="flex items-center gap-1 text-[10px] text-slate-500 leading-snug">
      <span>{c.label}</span>
      <InfoTip>
        <div className="max-w-[260px]">
          <div className="font-medium text-slate-200 mb-1">Correlation (r)</div>
          <div>Shows the strength and direction of a linear relationship. +1 means the two series move closely together, 0 means little linear relationship, and −1 means they tend to move in opposite directions.</div>
          <div className="mt-1">It does not show causation, and there is no universally “good” or “bad” value.</div>
        </div>
      </InfoTip>
    </div>
    <div className="flex items-baseline gap-2 mt-1">
      <span className={`font-mono text-lg font-semibold shrink-0 ${c.r > 0 ? "buy" : c.r < 0 ? "sell" : "text-slate-400"}`}>{c.r ?? "—"}</span>
      <div className="min-w-0">
        <div className="text-[10px] font-mono text-slate-500">{c.n} observations</div>
        <div className={`text-[10px] font-mono capitalize ${c.r > 0 ? "text-emerald-300" : c.r < 0 ? "text-red-300" : "text-slate-400"}`}>{rDesc(c.r)} relationship</div>
      </div>
    </div>
  </div>
);
